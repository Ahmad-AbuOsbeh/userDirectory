const stringsUI = {
	...
	_setupTinyMCE() {
		...
		tinymce.init({
			...
			plugins: ['advlist autolink lists link image charmap print preview anchor textcolor', 'searchreplace visualblocks code fullscreen', 'insertdatetime media table paste code colorpicker'],
			...
			content_css: ['https://fonts.googleapis.com/css?family=Lato:300,300i,400,400i', 'https://www.tiny.cloud/css/codepen.min.css'],
			setup: function(ed) {
	...
	buildLabel(container, prop, labelObj) {
		...
		if (labelObj.inputType && ['textarea', 'wysiwyg'].indexOf(inputType) >= 0) inputElement = this.createAndAppend('textarea', '', ['form-control', 'bf' + inputType], div);
		else {
